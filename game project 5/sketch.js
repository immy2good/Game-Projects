...
function setup()
{
	...
	sun_y = height/4;
	numTrees = 3;
	canyon = {x_pos: 200, y_pos : floorPos_y, size_x: 100, size_y: 150};
	collectables = [
		{x_pos : random(50, width-50), y_pos: floorPos_y-random(0,50), size: 40, isFound: false, reappearTime: 5000},
		{x_pos : random(50, width-50), y_pos: floorPos_y-random(0,50), size: 40, isFound: false, reappearTime: 5000},
		{x_pos : random(50, width-50), y_pos: floorPos_y-random(0,50), size: 40, isFound: false, reappearTime: 5000}
	]
...
function keyPressed()
{
	...
	if ((keyCode == 87 || keyCode ==38) && !isFalling) // w
		{
...
function drawCollectable(t_collectable) {
    if (!t_collectable.isFound) {
        ...
        strokeWeight(6);
        stroke(220, 185, 0);
        ellipse(t_collectable.x_pos + scrollPos, t_collectable.y_pos - 20, 
...
// 	if (dist(gameChar_x, gameChar_y, t_collectable.x_pos + scrollPos, t_collectable.y_pos) < 10)
// 		{
...
function checkCollectable(t_collectable) {
	...
	// if (t_collectable.isFound && t_collectable.collectableTimer !== null )	
	// {
	// 	if (millis() - collectableTimer >= t_collectable.reappearTime) {
	// 		t_collectable.isFound = false; // TODO: Need to Fix this . It reappears but its not collectable
...
function drawCharacterLeft() {
	fill(255, 0, 0);
		...
		rect(gameChar_x - 20, gameChar_y - 10, 10, 10);
		//Right leg
		rect(gameChar_x + 10, gameChar_y - 10, 10, 10);s
}